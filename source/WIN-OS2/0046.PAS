...
I've made a unit (Huge) that uses the global-heap to allocate
memory for arrays. The size of the arrays are only limited by
hardware memory.
}
Unit Huge;
...
{$IFOPT R+}
       if (X<MinX) or (X>MaxX) then
          if id_No=MessageBox(0,'Error in huge array, continue?','Index out of range!',
...
{$IFOPT R+}
       if (X<MinX) or (X>MaxX) or (Y<MinY) or (Y>MaxY) then
          if id_No=MessageBox(0,'Error in huge matrix, continue?','Index out of range!',
                              mb_YesNo or mb_IconQuestion) then
          begin
             Done;
             Halt;
...
begin
   Arr:=new(PHugeRealArray,Init(0,Max,
            SizeOf(Real),true));
   if Arr^.Data<>nil then
   begin
      RandSeed:=1;
      ...
      RandSeed:=1;
      for J:=0 to 50 do
         writeln(Arr^.I(random(Max))^:5:1);
   end;
   Arr^.Done;
   Mat:=new(PHugeWordMatrix,Init(0,MaxMat,0,MaxMat,
            SizeOf(Word),true));
   if Mat^.Data<>nil then
   begin
      RandSeed:=1;
       for J:=0 to 50 do
         Mat^.I(random(MaxMat),random(MaxMat))^:=J;
      RandSeed:=1;
      for J:=0 to 50 do
         writeln(Mat^.I(random(MaxMat),random(MaxMat))^);
   end;
   Mat^.Done;
   writeln('Done');
end.
